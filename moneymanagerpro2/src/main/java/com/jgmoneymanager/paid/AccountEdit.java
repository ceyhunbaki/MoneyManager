...
public class AccountEdit extends MyActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		View child = inflater.inflate(R.layout.accountedit, null);
		...
		if (savedInstanceState != null) {
			...
			else 
			{
				...
				initialBalance = Tools.formatDecimal(0d);
				description = "";			
	...
	private void reloadScreen() {
		...
		TextView lbAddedDate = (TextView)findViewById(R.id.lbAccAddedDateValue);
		...
		edInitialBalance.addTextChangedListener(new TextWatcher() {
			...
			@Override
			public void afterTextChanged(Editable s) {
                if (edInitialBalance.getText().toString().length() != 0)
                    try{
                        initialBalance = Tools.formatDecimal(Double.parseDouble(edInitialBalance.getText().toString()));
                    }
                    catch (Exception e) {
                        DialogTools.toastDialog(AccountEdit.this, R.string.msgInvalidNumber, Toast.LENGTH_SHORT);
                        initialBalance = Tools.formatDecimal(0d);
                    }
                else initialBalance = Tools.formatDecimal(0d);
		...
		if (resultCode == RESULT_OK)
		{
			if (requestCode == Constants.RequestCurrencyForAccount) {
				...
				btCurrSign.setText(DBTools.getCursorColumnValue(cursor, CurrencyTableMetaData.NAME) + 
						": " + /*CurrencySrv.getCurrencySymbol(*/DBTools.getCursorColumnValue(cursor, CurrencyTableMetaData.SIGN));
			}
			else if (requestCode == Constants.RequestCalculator) {
				//initialBalance = Tools.formatDecimal(data.getDoubleExtra(Constants.calculatorValue, 0d));
				((EditText) findViewById(R.id.edAccInitBalance)).setText(Tools.formatDecimal(data.getDoubleExtra(Constants.calculatorValue, 0d)));
			}
		}	
	}
	...
	public void myClickHandler(View target) {
		Intent intent;
		switch (target.getId()) {
			case R.id.btAccNameEd:
				(findViewById(R.id.edAccName)).requestFocus();
				break;
			case R.id.btAccCurrSignEd:
				(findViewById(R.id.btAccCurrSign)).performClick();
				break;
			case R.id.btAccInitBalanceEd:
				intent = new Intent(getBaseContext(), Calculator.class);
				if (Double.compare(Tools.stringToDouble(AccountEdit.this, initialBalance, false), 0) != 0) {
					Bundle bundle = new Bundle();
					bundle.putDouble(Calculator.startupAmountKey, Tools.stringToDouble(AccountEdit.this, initialBalance, false));
				...
				break;
			case R.id.btAccDescEd:
				(findViewById(R.id.edAccDesc)).requestFocus();
				break;
			case R.id.btAccStatusEd:
				(findViewById(R.id.spAccStatus)).performClick();
				break;
			...
			case R.id.btAccOk:
				...
				String name = Tools.cutName(edName.getText().toString()).replace("'", "");
				...
				values.put(AccountTableMetaData.INITIALBALANCE, Tools.formatDecimalUser2DB(AccountEdit.this, edInitialBalance.getText().toString()));
				...
				if (getIntent().getAction().equals(Intent.ACTION_INSERT)) {
					...
					if (!CurrRatesSrv.rateExists(AccountEdit.this, oldCurrID, newCurrID, rateDate, sb, null)) {
						Command cmd = new Command() {
							@Override
							public void execute() {
								AccountSrv.updateAccount(AccountEdit.this, values, oldInitialBalance,
										Tools.formatDecimalUser2DB(AccountEdit.this, edInitialBalance.getText().toString()), newCurrID, oldCurrID, id);
								setResult(RESULT_OK);
								finish();
							}
						};
