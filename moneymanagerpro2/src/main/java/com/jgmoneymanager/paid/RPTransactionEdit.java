...
public class RPTransactionEdit extends MyActivity {
	...
	Spinner spRPTTransStatus;
	Spinner spRPTTransPaymMethod;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		View child = inflater.inflate(R.layout.rptransactionedit, null);
		...
		if (savedInstanceState != null) {
			...
			transactionStatus = Tools.getIntegerFromBundle0(savedInstanceState, "transactionStatus");
			transactionPaymMethod = Tools.getIntegerFromBundle0(savedInstanceState, "transactionPaymMethod");
			...
			else if (getIntent().getAction().equals(Intent.ACTION_EDIT))
			{
				...
				transactionStatus = DBTools.getCursorColumnValueInt(cursor, VTransferViewMetaData.TRANSACTION_STATUS);
				transactionPaymMethod = DBTools.getCursorColumnValueInt(cursor, VTransferViewMetaData.TRANSACTION_PAYMENT_METHOD);
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		Tools.putToBundle(outState, "transactionStatus",  transactionStatus);
		Tools.putToBundle(outState, "transactionPaymMethod", transactionPaymMethod);
	...
	private void reloadScreen() {
		...
		ArrayAdapter<String> spinnerArrayAdapter = new ArrayAdapter<String>(this, R.layout.simple_spinner_item, R.id.spinItem, getResources().getStringArray(R.array.RPTransTypes));
		...
		cursor = this.getContentResolver().query(TransactionStatusTableMetaData.CONTENT_URI, null, null, null, null);
		spRPTTransStatus = ((Spinner) findViewById(R.id.spRtpTransStatus));
		LocalTools.fillSpinner(spRPTTransStatus, this, cursor, TransactionStatusTableMetaData.NAME);
		if (transactionStatus != 0) {
			int position = 0;
			for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
				if (DBTools.getCursorColumnValueLong(cursor, TransactionStatusTableMetaData._ID) == transactionStatus) {
					spRPTTransStatus.setSelection(position);
					break;
				}
				position++;
			}
		}
		spRPTTransStatus.setOnItemSelectedListener(new OnItemSelectedListener() {
			public void onItemSelected(AdapterView<?> arg0, View arg1,
									   int arg2, long arg3) {
				transactionStatus = (int) spRPTTransStatus.getSelectedItemId();
			}
			public void onNothingSelected(AdapterView<?> arg0) {
			}
		});
		...
		spRPTTransPaymMethod = ((Spinner) findViewById(R.id.spRtpTransPaymentMethod));
		LocalTools.fillSpinner(spRPTTransPaymMethod, this, cursor, PaymentMethodsTableMetaData.NAME);
		if (transactionPaymMethod != 0) {
			int position = 0;
			for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
				if (DBTools.getCursorColumnValueLong(cursor, PaymentMethodsTableMetaData._ID) == transactionPaymMethod) {
					spRPTTransPaymMethod.setSelection(position);
					break;
				}
				position++;
			}
		}
		spRPTTransPaymMethod.setOnItemSelectedListener(new OnItemSelectedListener() {
			public void onItemSelected(AdapterView<?> arg0, View arg1,
									   int arg2, long arg3) {
				transactionPaymMethod = (int) spRPTTransPaymMethod.getSelectedItemId();
			}
			public void onNothingSelected(AdapterView<?> arg0) {
			}
		});
	...
	private void insertRPTransaction(Context context, ContentValues values)
	{
		...
		RPTransactionSrv.insertTransactionsFromRPTransaction(getBaseContext(), 
				...
				values.getAsInteger(TransferTableMetaData.TRANSACTION_STATUS),
				values.getAsInteger(TransferTableMetaData.TRANSACTION_PAYMENT_METHOD));
	}
	...
	private static void updateTransactionFromRPTransaction(Context context, long oldAccountId, long newAccountId, 
			...
			int newTransactionStatus, int newTransactionPaymMethod)
	{
				...
				(newRepeatType == Constants.TransferType.Once.index())))
		{
			...
			for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext())
			{
				TransactionSrv.updateTransaction(context, DBTools.getCursorColumnValue(cursor, TransactionsTableMetaData._ID), 
					...
					oldCurrID, newCurrID, 0, 0, transferID, null, newTransactionStatus, newTransactionPaymMethod);
			}
		}
		else  
		{
			TransferEdit.deleteTransactionsFromTransfer(context, transferID);
			RPTransactionSrv.insertTransactionsFromRPTransaction(context, newAccountId, transactionType, newTransDate, newAmount,
					description, newRepeatType, newCustomInterval, newPeriodEnd, transferID, newCurrID, newCategoryID,
					newTransactionStatus, newTransactionPaymMethod);
		}
	}
	...
	public static void generateRPTransNotification(Context context) {
		...
		if (notification.length() > 0) {
			...
			DialogTools.systemNotification(context, notificationIntent, 1, context.getString(R.string.app_name_pro),
			...
			Command yesCommand = new Command() {
				public void execute() {
					updateTransactionFromRPTransaction(getBaseContext(), oldAccountID, accountID, 
							...
							updatedID, oldCurrID, currID, oldCategoryID, categoryID, transactionStatus, transactionPaymMethod);
    ...
    public void myClickHandler(View target) {
		Intent intent;
		switch (target.getId()) {
			case R.id.btRtpAccountEd:
				findViewById(R.id.spRtpAccount).performClick();
				break;
			case R.id.btRtpRPTransTypeEd:
				findViewById(R.id.spRtpRPTransType).performClick();
				break;
			case R.id.btRtpCategoryEd:
				findViewById(R.id.btRtpCategory).performClick();
				break;
			case R.id.btRtpTransDateEd:
				findViewById(R.id.btRtpTransDate).performClick();
				break;
			case R.id.btRtpDescriptionEd:
				findViewById(R.id.edRtpDescription).requestFocus();
				break;
			case R.id.btRtpRepeatEd:
				findViewById(R.id.spRtpRepeat).performClick();
				break;
			case R.id.btRtpPeriodEndEd:
				findViewById(R.id.btRtpPeriodEnd).performClick();
				break;
			case R.id.btRtpCustomIntervalEd:
				findViewById(R.id.edRtpCustomInterval).requestFocus();
				break;
			case R.id.btRtpCurrencyEd:
				findViewById(R.id.btRtpCurrency).performClick();
				break;
			case R.id.btRtpTransStatusEd:
				findViewById(R.id.spRtpTransStatus).performClick();
				break;
			case R.id.btRtpTransPaymentMethodEd:
				findViewById(R.id.spRtpTransPaymentMethod).performClick();
				break;
			...
			case R.id.btRtpOk:
				if ((amount == null) || (amount == 0)) {
					...
					values.put(TransferTableMetaData.TRANSACTION_STATUS, transactionStatus);
					values.put(TransferTableMetaData.TRANSACTION_PAYMENT_METHOD, transactionPaymMethod);
