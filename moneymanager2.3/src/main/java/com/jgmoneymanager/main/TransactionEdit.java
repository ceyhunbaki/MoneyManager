...
public class TransactionEdit extends MyActivity {
            ...
            if (getIntent().getAction().equals(Intent.ACTION_INSERT)) {
                ...
                switch (transactionType) {
                    case Constants.TransactionTypeExpence:
                        setCustomTitle(R.string.addExpence);
                        break;
                    case Constants.TransactionTypeIncome:
                        setCustomTitle(R.string.addIncome);
                        break;
                    default:
                        break;
                }
                ...
                setCustomTitle(R.string.editTransaction);
    ...
    void setCustomTitle(int stringID) {
        TextView tvTitle = (TextView) findViewById(R.id.tvATTitle);
    ...
    /**
     * If budget for selected category is avialable, then shows {@link R.id.tvTrProgressText} and sets text
     */
        ...
        if ((transactionType == Constants.TransactionTypeExpence)
            ...
            try {
                if (BudgetSrv.getCategoryRemainingBudget(getBaseContext(), categoryID, transDate, sbRemaining, sbBudget, sbUsed)) {
                    ...
                    pBar.setProgress((int) Math.round(Tools.stringToDouble(sbUsed.toString()) * 100 / Tools.stringToDouble(sbBudget.toString())));
                } else
    ...
    public void myClickHandler(View target) {
        ...
        try {
            switch (target.getId()) {
                /*case R.id.btTrAccount:
                    cursor = this.managedQuery(VTransAccountViewMetaData.CONTENT_URI, null,
                    ...
                    Command cmd = new Command() {
                        ...
                        public void execute() {
                            Cursor cursorInt = getContentResolver().query(VTransAccountViewMetaData.CONTENT_URI,
                                    new String[]{VTransAccountViewMetaData._ID, VTransAccountViewMetaData.NAME},
                                    VTransAccountViewMetaData.STATUS + " =1 ",
                                    null, null);
                            cursorInt.moveToPosition(Constants.cursorPosition);
                            accountID = DBTools.getCursorColumnValueInt(cursorInt, VTransAccountViewMetaData._ID);
                            btAccount.setText(DBTools.getCursorColumnValue(cursorInt, VTransAccountViewMetaData.NAME));
                            accountCurrID = AccountSrv.getCurrencyIdByAcocuntID(TransactionEdit.this, accountID);
                            refreshCurrency(TransactionEdit.this, true);
                        }
                    };
                    AlertDialog accountList = DialogTools.RadioListDialog(this, cmd, R.string.selectAccount, cursor, VTransAccountViewMetaData.NAME, true);
                    accountList.show();
                    break;*/
                case R.id.btTrAccountEd:
                    ((Spinner) findViewById(R.id.spTrAccount)).performClick();
                    break;
                ...
                case R.id.btTrPhoto:
                    ...
                    break;
                /*case R.id.btTrStatus:
                    intent = new Intent(getBaseContext(), TransactionStatusList.class);
                    intent.setAction(Intent.ACTION_PICK);
                    startActivityForResult(intent, Constants.RequestStatus);
                    break;*/
                case R.id.btTrStatusEd:
                    ((Spinner) findViewById(R.id.spTrStatus)).performClick();
                    break;
                case R.id.btTrMethodEd:
                    ((Spinner) findViewById(R.id.spTrMethod)).performClick();
                    break;
                case R.id.btTrOk:
                    ...
                    if ((amount == null) || (amount.length() == 0) || amount.equals(".") || amount.equals(",")) {
                        ...
                        CurrRatesSrv.insertRate(TransactionEdit.this, currID, accountCurrID, Tools.parseDouble(edRate.getText().toString()), transDate);
                        if (getIntent().getAction().equals(Intent.ACTION_INSERT)) {
                            TransactionSrv.insertTransaction(getBaseContext(), accountID, categoryID, transDate, Tools.stringToDouble(amount), transactionType,
                                        ...
                                        try {
                                            ...
                                            if (otherTransactionID != 0)
                                                TransactionSrv.updateTransaction(TransactionEdit.this, String.valueOf(otherTransactionID),
                                                        ...
                                                        oldAmount, Tools.parseDouble(((EditText) findViewById(R.id.edTrAmount)).getText().toString()), -transactionType,
                    ...
                    DropboxCheckRevisionForUpload dvTask = new DropboxCheckRevisionForUpload(TransactionEdit.this, file);
                    dvTask.execute();*/
                    break;
